...
| `Calendars.ReadWrite` (Delegated) | ตรวจห้องว่างและจองห้องผ่านเว็บ · ขอเฉพาะตอนกดจอง | ถ้าใช้ฟอร์มจองในเว็บ |
...
คลังเอกสารของไซต์
├── Photos/
│   ├── Directory/<ชื่อฝ่าย>/        รูปบุคลากร
│   └── Announcements/<ชื่อฝ่าย>/    รูปประกาศเด้งหน้าแรก
└── Attachments/
    ├── Policies/<ชื่อฝ่าย>/         ไฟล์แนบนโยบายบริษัท
    ├── News/                        ไฟล์แนบข่าวประกาศ
    └── Documents/<ชื่อฝ่าย>/        ไฟล์แนบเอกสารและคู่มือ
```

ตัวอย่างเส้นทางจริง

```
Attachments/Documents/ฝ่ายความปลอดภัย อาชีวอนามัยและสิ่งแวดล้อม/คู่มือความปลอดภัย-a1b2c3.pdf
Photos/Directory/ผู้บริหาร/ไพรัช-เขียนเขว้า-d4e5f6.jpg
```

ชื่อไฟล์ต่อท้ายด้วยรหัสสุ่ม เพื่อไม่ให้ทับกันเมื่อชื่อซ้ำ และคงนามสกุลเดิมไว้
รายการที่ไม่ได้ระบุฝ่ายจะไปอยู่โฟลเดอร์ `ไม่ระบุฝ่าย`

**ข้อควรรู้** เมื่อย้ายเอกสารหรือบุคลากรไปฝ่ายอื่น ระบบเปลี่ยนเฉพาะค่าในคอลัมน์
ไฟล์เดิมยังอยู่โฟลเดอร์เดิม ลิงก์ยังใช้ได้ปกติแต่ตำแหน่งไฟล์จะไม่ตรงกับฝ่ายใหม่
ถ้าต้องการให้ตรง ให้ลบไฟล์แล้วอัปโหลดใหม่หลังเปลี่ยนฝ่าย

### L18 — ProjectHistory (ประวัติความคืบหน้าโครงการ)

ระบบเขียนให้เองทุกครั้งที่ผู้ดูแลแก้ไขความคืบหน้าโครงการ โดยเก็บ **ค่าก่อนแก้** ไว้
ทำให้ย้อนดูได้ว่าแต่ละเดือนคืบหน้าไปเท่าไร โดยที่หน้าแดชบอร์ดยังแสดงค่าล่าสุดเสมอ

| คอลัมน์ | ชนิด | หมายเหตุ |
|---|---|---|
| Title | Single line of text | ชื่อโครงการ ณ ตอนนั้น |
| ProjectCode | Single line of text | รหัสโครงการ ใช้จับคู่กับ L16 |
| RecordedDate | Date and Time | วันที่ของข้อมูลชุดนั้น |
| PlanProgress | Number | |
| ActualProgress | Number | |
| ActualPayment | Number | |
| Detail | Multiple lines of text (plain) | |
| RecordedBy | Single line of text | ชื่อผู้ที่แก้ไข |

ทะเบียนนี้ไม่ต้องกรอกเอง หน้าจัดการข้อมูลเปิดให้ดูอย่างเดียว ไม่มีปุ่มเพิ่มหรือลบ

### L17 — Feedback (กล่องรับฟังความคิดเห็น)

รับข้อเสนอแนะจากหน้าติดต่อ

| คอลัมน์ | ชนิด | หมายเหตุ |
|---|---|---|
| Title | Single line of text | เรื่องที่เสนอ |
| Content | Multiple lines of text (plain) | รายละเอียด |
| SubmittedBy | Single line of text | ชื่อผู้เสนอ เว้นว่างแปลว่าเลือกไม่ระบุชื่อ |
| SubmittedEmail | Single line of text | อีเมลผู้เสนอ |
| IsAnonymous | Yes/No | |
| Status | Choice | `ยังไม่ได้อ่าน` / `รับทราบแล้ว` / `กำลังดำเนินการ` / `ดำเนินการแล้ว` / `ไม่ดำเนินการ` |
| Reply | Multiple lines of text (plain) | บันทึกการดำเนินการ ผู้ส่งไม่เห็น |

**เรื่องการไม่ระบุชื่อ** — ระบบไม่บันทึกชื่อลงในคอลัมน์เมื่อผู้ส่งเลือกไม่ระบุ
แต่ SharePoint บันทึก Created By ไว้เองทุกรายการ จึงไม่ใช่การปกปิดชื่อโดยสมบูรณ์

หากต้องการให้ไม่ระบุชื่อจริง ๆ ต้องใช้ Power Automate เขียนข้อมูลแทนผู้ใช้
ซึ่งอยู่นอกขอบเขตของเว็บฝั่งหน้าบ้าน หน้าเว็บเขียนข้อความนี้บอกผู้ใช้ไว้แล้ว
...
ลำดับของตัวเลือกใน SharePoint คือลำดับชั้นบนผัง เรียงจากบนลงล่าง
...
| Capacity | Number | ขนาดติดตั้ง kWp · ใส่ 0 สำหรับงานที่ไม่คิดเป็นกำลังผลิต |
| StartDate | Date | วันเริ่มโครงการ |
| EndDate | Date | วันสิ้นสุดตามสัญญา |
