...
export const SCHEMA = {
  ...
  projects: {
    ...
    fields: [
      ...
      { key: 'PlanProgress', label: 'ความคืบหน้าตามแผน (%)', type: 'number', step: '0.01', min: 0, max: 100,
        help: 'ใส่ทศนิยมได้ 2 ตำแหน่ง เช่น 45.75' },
      { key: 'ActualProgress', label: 'ความคืบหน้าจริง (%)', type: 'number', step: '0.01', min: 0, max: 100,
        help: 'ใส่ทศนิยมได้ 2 ตำแหน่ง เช่น 45.75' },
      { key: 'ActualPayment', label: 'เบิกจ่ายแล้ว (%)', type: 'number', step: '0.01', min: 0, max: 100,
        help: 'ใส่ทศนิยมได้ 2 ตำแหน่ง เช่น 45.75' },
      ...
      { key: 'Team', label: 'ทีมงาน (เลือกได้หลายคน)', type: 'multilookup', from: 'directory',
        labelWith: 'Position', searchable: true, full: true,
        help: 'ผู้ร่วมรับผิดชอบโครงการ แสดงพร้อมรูปในหน้ารายละเอียด' },
      { key: 'Detail', label: 'ขณะนี้ดำเนินการอะไรอยู่', type: 'textarea', full: true,
        help: 'เขียนสิ่งที่กำลังทำและอุปสรรคที่พบ ข้อความนี้แสดงบนแดชบอร์ด' },
      { key: 'InWarranty', label: 'อยู่ระหว่างประกันผลงาน (O&M Scope)', type: 'yesno',
        help: 'ติ๊กเมื่อส่งมอบแล้วและอยู่ในช่วงรับประกัน · จะแสดงแทนเวลาที่เหลือ' },
      { key: 'WarrantyEnd', label: 'ประกันผลงานถึงวันที่', type: 'date',
        help: 'ทางเลือก · ถ้าระบุ จะนับวันที่เหลือของช่วงประกันให้' },
  ...
  projectHistory: {
    ...
    fields: [
      ...
      { key: 'PlanProgress', label: 'แผน (%)', type: 'number', step: '0.01', min: 0, max: 100,
        help: 'ใส่ทศนิยมได้ 2 ตำแหน่ง เช่น 45.75' },
      { key: 'ActualProgress', label: 'จริง (%)', type: 'number', step: '0.01', min: 0, max: 100,
        help: 'ใส่ทศนิยมได้ 2 ตำแหน่ง เช่น 45.75' },
      { key: 'ActualPayment', label: 'เบิกจ่าย (%)', type: 'number', step: '0.01', min: 0, max: 100,
        help: 'ใส่ทศนิยมได้ 2 ตำแหน่ง เช่น 45.75' },
  ...
  directory: {
    ...
    fields: [
      ...
      { key: 'EmployeeCode', label: 'รหัสบุคลากร', type: 'text',
        help: 'ใช้กรอกลงแบบฟอร์มอัตโนมัติ ไม่แสดงบนหน้าบุคลากร' },
      ...
      { key: 'Oversees', label: 'ดูแลฝ่าย (เลือกได้หลายฝ่าย)', type: 'multilookup', from: 'departments', full: true,
      ...
      { key: 'Manager', label: 'ผู้บังคับบัญชา (หัวหน้าโดยตรง)', type: 'lookup', from: 'directory',
        valueField: 'Title', labelWith: 'Position', searchable: true,
        help: 'ใช้กับขั้นอนุมัติแบบ "ผู้บังคับบัญชาของผู้ยื่น" · เลือกจากทะเบียนบุคลากร' },
      { key: 'BackupManager', label: 'ผู้อนุมัติแทน (สำรอง)', type: 'lookup', from: 'directory',
        valueField: 'Title', labelWith: 'Position', searchable: true,
        help: 'ทางเลือก · ใช้แทนเมื่อหัวหน้าไม่อยู่ (ระบบยังไม่สลับให้อัตโนมัติ)' },
      { key: 'StartDate', label: 'วันเริ่มงาน', type: 'date',
        help: 'ใช้คำนวณกำหนดประเมินทดลองงาน 30 / 60 / 90 / 120 วัน และเติมลงใบประเมินผ่านงานอัตโนมัติ' },
  ...
  formCatalog: {
    ...
    fields: [
      ...
      { key: 'FormCode', label: 'รหัสฟอร์มภายในระบบ', type: 'text', required: true,
        help: 'ตัวเชื่อมกับช่องกรอก · ตั้งครั้งเดียวแล้วไม่ควรเปลี่ยน เช่น FM-HR-001' },
      { key: 'ISODocNo', label: 'เลขที่เอกสาร ISO', type: 'text',
        help: 'เลขที่ตามระบบ ISO 9001 แสดงบนหัวฟอร์ม เปลี่ยนได้เมื่อมีการรีวิชั่น' },
      { key: 'ISORevision', label: 'ฉบับแก้ไข (Revision)', type: 'text',
  ...
  approvalMatrix: {
    ...
    fields: [
      ...
      { key: 'Approvers', label: 'ระบุชื่อผู้อนุมัติ (เลือกได้หลายคน)', type: 'multilookup',
        ...
        showIf: 'ApproverType=ระบุชื่อเจาะจง',
        help: 'ใช้เมื่อเลือกระบุชื่อเจาะจง · ใส่ได้ 2-3 คนต่อลำดับ' },
  ...
  formFields: {
    ...
    labels: { FormCode: 'รหัสฟอร์ม', Title: 'ชื่อช่อง', FieldType: 'ชนิด',
              Section: 'หัวข้อ', IsRequired: 'บังคับ' },
    fields: [
      { key: 'FormCode', label: 'แบบฟอร์ม', type: 'lookup', from: 'formCatalog',
        valueField: 'FormCode', labelWith: 'Title', searchable: true, required: true,
        help: 'เลือกจากฟอร์มที่มีจริง จะได้ไม่พิมพ์รหัสผิด' },
      { key: 'FieldKey', label: 'ชื่อช่องในระบบ', type: 'text', required: true,
        help: 'อังกฤษตัวเล็กและขีดล่าง เช่น incident_date · ห้ามซ้ำในฟอร์มเดียวกัน' },
      { key: 'Title', label: 'ชื่อช่องที่ผู้ใช้เห็น', type: 'text', required: true },
      { key: 'FieldType', label: 'ชนิดช่อง', type: 'choice',
        options: ['text', 'textarea', 'number', 'currency', 'date', 'time', 'choice',
                  'multichoice', 'lookup', 'person', 'yesno', 'file', 'readonly', 'lineitems'] },
      { key: 'Options', label: 'ตัวเลือก', type: 'textarea', full: true,
        help: 'choice/multichoice: บรรทัดละตัวเลือก · lineitems: บรรทัดละคอลัมน์ รูปแบบ key|ชื่อ|ชนิด (ชนิด text/number/money)' },
      { key: 'IsRequired', label: 'บังคับกรอก', type: 'yesno' },
      { key: 'DefaultValue', label: 'ค่าเริ่มต้น', type: 'text',
        help: '{today} = วันนี้ · {me.Title} {me.Position} {me.Department} {me.EmployeeCode} = ข้อมูลผู้ยื่น' },
      { key: 'HelpText', label: 'คำอธิบายใต้ชื่อช่อง', type: 'text', full: true },
      { key: 'Section', label: 'หัวข้อที่จัดกลุ่ม', type: 'text',
        help: 'ช่องที่ใส่หัวข้อเดียวกันจะอยู่กลุ่มเดียวกัน' },
      { key: 'ColumnWidth', label: 'ความกว้าง', type: 'choice', options: ['half', 'full'] },
      { key: 'ShowIf', label: 'แสดงเมื่อ', type: 'text', full: true,
        help: 'รูปแบบ ชื่อช่อง=ค่า · หลายเงื่อนไขคั่นด้วย | เช่น subject=เรื่องอื่น ๆ' },
      { key: 'IsActive', label: 'เปิดใช้งาน', type: 'yesno' },
    ],
  },
  ...
  policies: {
    ...
    fields: [
      ...
      { key: 'Department', label: 'ฝ่ายเจ้าของ', type: 'lookup', from: 'departments' },
      { key: 'Content', label: 'เนื้อหานโยบาย', type: 'textarea', full: true,
        help: 'พิมพ์เนื้อหาเต็มได้ที่นี่ ผู้ใช้กดชื่อเอกสารแล้วจะเห็น' },
      { key: 'Files', label: 'ไฟล์แนบ', type: 'files',
        help: 'แนบได้หลายไฟล์ · รับทุกชนิดไฟล์ ไฟล์ละไม่เกิน 1 GB · ระบบอ่านชนิดและขนาดไฟล์ให้เอง' },
  ...
  news: {
    ...
    fields: [
      ...
      { key: 'Content', label: 'เนื้อหา', type: 'textarea', full: true },
  ...
  appraisalCycles: {
    ...
    fields: [
      { key: 'Title', label: 'ชื่อแบบประเมิน', type: 'text', required: true,
        help: 'เช่น ปี 2569 ครึ่งปีแรก' },
      { key: 'FormSet', label: 'ชุดแบบประเมินที่ใช้', type: 'choice', required: true,
        options: ['ประเมินบุคลากรผ่านงาน (FM-HRM-004)', 'ประเมิน KPI ประจำปี'],
        help: 'แบบผ่านงานมีหัวข้อมาตรฐานติดมากับระบบแล้ว ไม่ต้องเพิ่มหัวข้อเอง' },
      { key: 'Round', label: 'ประเมินครั้งที่', type: 'choice',
        options: ['ครั้งที่ 1 (30 วัน)', 'ครั้งที่ 2 (60 วัน)', 'ครั้งที่ 3 (90 วัน)', 'ครั้งที่ 4 (120 วัน)'],
        custom: 'อื่นๆ / กำหนดเอง', customHint: 'เช่น รอบเดือนมกราคม 2570, ไตรมาส 1/2570',
        help: 'ครั้งที่ 1–4 ใช้กับแบบผ่านงาน · แบบอื่น เช่น KPI เลือก "อื่นๆ / กำหนดเอง" แล้วพิมพ์รอบเอง' },
      { key: 'RoundDate', label: 'วันที่ประเมินครั้งนี้', type: 'date',
        help: 'ระบบใส่วันนี้ลงช่องประเมินครั้งที่เลือกของทุกคนในตารางด้านล่าง · ครั้งอื่นกรอกเองในตาราง' },
      { key: 'Status', label: 'สถานะ', type: 'choice', options: ['เปิด', 'ปิด'], required: true },
      { key: 'Scope', label: 'ผู้ถูกประเมิน', type: 'choice', options: ['บุคลากรทุกคน', 'เฉพาะรายชื่อที่ระบุ'],
        help: 'ประเมินผ่านงานมักใช้เฉพาะรายชื่อที่ระบุ (พนักงานใหม่ในรอบนั้น)' },
      { key: 'Members', label: 'รายชื่อผู้ถูกประเมิน', type: 'people', full: true,
        help: 'ใช้เมื่อเลือก "เฉพาะรายชื่อที่ระบุ" · กรองตามฝ่าย ค้นหาชื่อ แล้วติ๊กเลือกได้' },
  ...
  appraisalCriteria: {
    ...
    columns: ['FormSet', 'Section', 'Title', 'Weight'],
    labels: { FormSet: 'ชุดแบบประเมิน', Section: 'หมวด', Title: 'หัวข้อ',
              Weight: 'น้ำหนัก (%)', SectionWeight: 'น้ำหนักหมวด (%)' },
    fields: [
      { key: 'FormSet', label: 'ชุดแบบประเมิน', type: 'choice', required: true,
        options: ['ประเมินบุคลากรผ่านงาน (FM-HRM-004)', 'ประเมิน KPI ประจำปี'],
        help: 'เว้นว่างได้ถ้าใช้ชุดมาตรฐานของแบบผ่านงาน' },
      ...
      { key: 'Weight', label: 'น้ำหนักของหัวข้อ (%)', type: 'number', step: '0.01', min: 0, max: 100, required: true },
      { key: 'Scale', label: 'สเกลให้คะแนน', type: 'choice', options: ['5', '10'],
        help: 'แบบทดลองงาน FM-HRM-004 ใช้ 10 · แบบทั่วไปใช้ 5' },
  ...
  appraisals: {
    ...
    columns: ['CycleName', 'FormSet', 'EmployeeName', 'Department', 'EvaluatorName', 'Status', 'FinalScore', 'Grade'],
    labels: { CycleName: 'แบบประเมิน', FormSet: 'ชุดแบบประเมิน', EmployeeName: 'ผู้ถูกประเมิน', Department: 'ฝ่าย',
